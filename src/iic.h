...
typedef struct {
	uint8_t mfsch;			/* MFS channel to be used as I2C (0-7) */
	uint8_t locno;			/* Pin relocation number (0-2) */
...
void i2c_init (volatile I2CCTRL *, uint32_t);	/* Initialize I2C module */
int i2c_start (volatile I2CCTRL *);				/* Start an I2C transaction */
void i2c_abort (volatile I2CCTRL *);			/* Abort an I2C transaction in progress */
